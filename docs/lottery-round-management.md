...
- The impact dashboard shows active round id, entries, round pool, and winner request state.
- A demo operator can close the round, request winner selection, and open the next round.
...
The next issue should replace the demo winner request state with Chainlink VRF request and fulfillment logic. The closed round id and entry list become the input set for random winner selection.
